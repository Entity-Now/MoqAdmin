	...
	<div class="login-popup-container">
		...
		<template v-if="oauthChannel.includes(currentLogin)">
			<div
				...
				<div class="flex flex-col items-center mt-[10px] mb-[25px]">
					<div
						...
						class="relative w-[206px] h-[206px]">
						<img
							v-if="sanQrCodeUrl"
							:src="sanQrCodeUrl"
							alt="qr"
			...
			<div
				...
				class="oauth">
				<div
					class="flex justify-center items-center text-primary-default cursor-pointer hover-opacity">
					<icon
						name="el-icon-Iphone"
						size="18" />
					<div
...
<script setup lang="ts">
	...
	const getMiniLoginQrCode = async () => {
		...
		try {
			// Get miniprogram URL link
			const urlLink = await loginApi.miniLoginQr("login");

			// Generate QR code from URL link using a QR code library or service
			// For now, we'll use a QR code API service
			const qrCodeUrl = `https://api.qrserver.com/v1/create-qr-code/?size=214x214&data=${encodeURIComponent(
				urlLink
			)}`;
			sanQrCodeUrl.value = qrCodeUrl;

			// Note: We don't have a key from mini_qrcode endpoint
			// We'll need to use a different approach for polling
			// For now, set a placeholder key
			sanLoginKey.value = "mini_" + Date.now();
			sanExpire.value = 120;
			...
			// Polling logic - check for login status
			// Note: The mini_qrcode endpoint doesn't return a state key
			// This is a limitation - we may need backend support for proper polling
			timer = setInterval(async () => {
				if (sanExpire.value) {
					sanExpire.value = sanExpire.value - 1;
					// TODO: Need backend to provide state key for miniprogram login polling
					// For now, we'll just show expiration
				} else {
					sanCodeStatus.value = 1;
					clearInterval(timer);
					timer = null;
