...
export function createRequest(opt?: Partial<FetchOptions>): Request {
	...
	const defaultOptions: FetchOptions = {
		...
		baseURL: (runtimeConfig.public.apiUrl as string) || config.baseUrl,
		headers: {
		...
		requestOptions: {
			...
			requestInterceptorsHook(options: any) {
				...
				const apiPrefix: string =
					options.requestOptions?.apiPrefix || "";
				const withToken: boolean =
					options.requestOptions?.withToken || false;
				const isShowLoading: boolean =
					options.requestOptions?.isShowLoading || false;
				const isParamsToData: boolean =
					options.requestOptions?.isParamsToData || false;
