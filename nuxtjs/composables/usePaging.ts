...
export function usePaging<T = any>(options: Options) {
	...
	// 请求分页接口
	const queryLists = async (isAppend: boolean = false): Promise<any> => {
		pager.loading = true;
		try {
			...
			if (!res?.lists) {
				...
				if (isAppend) {
					pager.lists = [...pager.lists, ...res.lists];
				} else {
			...
			// 判断是否已加载完所有数据
			if (pager.lists.length >= pager.total) {
				pager.finished = true;
			} else {
