<template>
	<el-form
		...
		<el-card
			...
			<el-form-item
				label="脚本列表"
				prop="website.scripts">
				<div class="w-[380px] space-y-3">
					<!-- 1. 循环绑定索引 index，通过 formData.website.scripts[index] 操作原数组 -->
					<div
						v-for="(script, index) in formData.website.scripts"
						:key="index"
						class="flex items-end gap-2">
						<!-- 2. v-model 绑定到数组的具体索引位置 -->
						<el-input
							type="textarea"
							v-model="formData.website.scripts[index]"
							placeholder="https://hm.baidu.com/hm.js?d16da1a2f..."
							class="flex-1" />
						<!-- 可选：添加删除按钮 -->
						<el-button
		...
		<el-card
			...
			<el-form-item
				...
				<div class="w-[380px]">
					<el-input
						v-model.trim="formData.pc.wechat"
						show-word-limit />
				</div>
			</el-form-item>
			<el-form-item
				...
				<div class="w-[380px]">
					<el-input
						v-model.trim="formData.pc.email"
						show-word-limit />
				</div>
			</el-form-item>
			<el-form-item
				...
				<div class="w-[380px]">
					<el-input
						v-model.trim="formData.pc.mobile"
						show-word-limit />
				</div>
			</el-form-item>
			<el-form-item
				...
				<div class="w-[380px]">
					<el-input
						v-model.trim="formData.pc.work_time"
						show-word-limit />
				</div>
			</el-form-item>
		</el-card>
		...
		<el-card
			...
			<el-form-item
				label="关闭访问"
				prop="h5.close_url"
				v-if="formData.h5.status === 1">
				<div class="w-80">
					<el-input
						v-model="formData.h5.close_url"
...
<script setup lang="ts">
	...
	const handleSubmit = async (): Promise<void> => {
		...
		loading.value = true;
		await basicsApi.save(formData).finally(() => {
			setTimeout(() => {
				loading.value = false;
			}, 500);
		});
	...
	// 3. 新增脚本方法
	const addScript = () => {
		formData.website.scripts.push("");
	};

	// 4. 删除脚本方法
	const removeScript = (index: number) => {
		if (formData.website.scripts.length <= 1) {
			ElMessage.warning("至少保留一个脚本");
			return;
		}
		formData.website.scripts.splice(index, 1);
	};
